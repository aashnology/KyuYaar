...
- **Provider choice (Featherless AI):** the model layer has a third adapter for Featherless AI's OpenAI-compatible API, so the same tools, guardrail and fallbacks run on an open-weight model. Retry, pacing and error handling are shared with the Gemini adapter, and its request and response handling is tested against a scripted transport (`tests/test_featherless.py`). It is selected explicitly with `KYUYAAR_PROVIDER=featherless`, or automatically only when no other provider has a key.
